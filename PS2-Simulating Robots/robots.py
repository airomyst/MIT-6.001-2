...
class RectangularRoom(object):
    ...
    def isTileCleaned(self, m, n):
        ...
        """
        if (m,n) not in self.tiles:
            raise ValueError('Tile not in Room')
    ...
    def getNumCleanedTiles(self):
        ...
        """
        cleanedTiles = 0
        for status in self.tiles.values():
            if status == 'cleaned':
                cleanedTiles += 1
        return cleanedTiles
...
class Robot(object):
    ...
    def __init__(self, room, speed):
        ...
        self.angle = round(random.uniform(0,360-0.01),2)
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        x = position.getX()
        y = position.getY()
        if (x >= self.room.width or x <= 0) or (y >= self.room.height or y <= 0):
            self.angle = round(random.uniform(0,360-0.01),2)
        else:
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        ...
        Robot = {}
        for iter_ in range(num_robots):
            Robot[iter_] = robot_type(room, speed)
        while (room.getNumCleanedTiles()/room.getNumTiles()) < min_coverage:
            for iter_ in range(num_robots):
                Robot[iter_].updatePositionAndClean()
            counter += 1
